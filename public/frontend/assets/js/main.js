...
(function ($) {
    ...
    $(".qty-plus-minus").prepend('<div class="dec ec_qtybtn">-</div>').append('<div class="inc ec_qtybtn">+</div>');

    // Click event for increment/decrement buttons
    $(document).on("click", ".ec_qtybtn", function (e) {
        e.preventDefault(); // Prevent default action

        var $qtybutton = $(this);
        var $qtyContainer = $qtybutton.closest('.qty-plus-minus');
        var $inputField = $qtyContainer.find("input.qty-input");
        var qtyOldValue = parseInt($inputField.val(), 10);

        if ($qtybutton.hasClass("inc")) {
            $inputField.val(qtyOldValue + 1);
        } else if ($qtybutton.hasClass("dec") && qtyOldValue > 1) {
            $inputField.val(qtyOldValue - 1);
        }

        // Trigger change event to update total price if needed
    ...
    $(document).on("click", ".ec_cart_qtybtn .ec_qtybtn", function () {
        var $cartqtybutton = $(this);
        var $input = $cartqtybutton.closest('.cart-qty-plus-minus').find("input.cart-plus-minus");
        var CartQtyoldValue = parseFloat($input.val());

        if ($cartqtybutton.hasClass("inc")) {
            var CartQtynewVal = CartQtyoldValue + 1;
        } else if ($cartqtybutton.hasClass("dec")) {
            var CartQtynewVal = CartQtyoldValue > 1 ? CartQtyoldValue - 1 : 1;
        }

        $input.val(CartQtynewVal).change(); // Trigger change event to update total
    });
